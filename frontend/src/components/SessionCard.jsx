...
const SessionCard = ({ session, onClick, onDelete }) => {
  ...
  const getIcon = () => {
    ...
    if (r.includes("Game")) return <Gamepad2 className="h-6 w-6" />;
  ...
  const getStatusClasses = () => {
    ...
    return "bg-violet-400/15 text-violet-300 border border-violet-400/20";
  };
  ...
  const getScoreColor = () => {
    ...
    return session.overallScore > 75 ? "text-emerald-300" : "text-orange-300";
  };

  return (
    <div
      onClick={() => onClick(session)}
      className="group cursor-pointer rounded-[2rem] border border-white/10 bg-white/5 p-5 backdrop-blur-xl transition-all duration-300 hover:-translate-y-1 hover:bg-white/[0.07] sm:p-6"
    >
      <div className="flex flex-col gap-4 md:flex-row md:items-center">
        <div className="flex min-w-0 flex-grow items-center gap-4 sm:gap-6">
          ...
          <div className="min-w-0">
            <h3 className="truncate text-base font-bold text-white transition group-hover:text-cyan-300 sm:text-lg">
              {session.role}
            </h3>
        ...
        <div className="flex w-full items-center justify-between border-t border-white/10 pt-4 md:w-auto md:justify-end md:gap-6 md:border-t-0 md:pt-0">
          ...
          <div className="flex flex-col items-end gap-2">
            ...
            <span className="flex items-center text-xs font-bold text-cyan-300">
